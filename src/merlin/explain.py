...
from merlin.util.helpers import union, jaccard_distance
from merlin.util.logger import build_logger
from merlin.util.BDD2Text import BDD2Text
...
class Explain():
    ...
    def __init__(self, data_manager,
        ...
        # self.rule_coverage = defaultdict(lambda: {
        #     'add': defaultdict(lambda: {'d_l': [], 'd_r': []}),
        #     'del': defaultdict(lambda: {'d_l': [], 'd_r': []}),
        #     'still': defaultdict(lambda: {'d_l': [], 'd_r': []}),
        # })
    ...
    def load_bdd_data(self):
        '''Load bdd data in a dictionary
        '''
        bdd_dict = defaultdict(lambda: list())
        if self.trace_results is None:
            self.trace_results = pd.read_csv(
                f'{self.save_path}/trace.csv', decimal=',', sep=';')
        for _, row in self.trace_results.iterrows():
    ...
    def _obdd_diff(self, class_id, s1, s2):
        ...
        f_add_bdd = expr2bdd(f_add)
        f_del_bdd = expr2bdd(f_del)
        f_still_bdd = expr2bdd(f_still)
        ...
        if isinstance(self.data_manager, TextDataManager):
            set_features_add = set_features_f - set_features_g
            set_features_del = set_features_g - set_features_f
            set_features_still = set_features_f.union(
                set_features_g) - set_features_f.intersection(set_features_g)
            ...
            self.logger.debug(
                f'SET FEATURES REMOVE STILL: {set_features_still}')
            self.logger.debug('-----------------------------------')
            self.logger.debug(
                f'SAT ADD PATHS BEFORE REMOVING DIFFS: {sat_add_lst}')
            self.logger.debug(
                f'SAT DEL PATHS BEFORE REMOVING DIFFS: {sat_del_lst}')
            self.logger.debug(
                f'SAT STILL PATHS BEFORE REMOVING DIFFS: {sat_still_lst}')
            self.logger.debug('-----------------------------------')

            sat_add_lst = self._remove_diffs(sat_add_lst, set_features_add)
            sat_del_lst = self._remove_diffs(sat_del_lst, set_features_del)
            sat_still_lst = [str({str(key): value for key, value in x.items(
            )}) for x in sat_still_lst if len(set(x.keys()).intersection(set_features_still)) == 0]

            sat_add_lst = {str(x) for x in sat_add_lst}
            sat_del_lst = {str(x) for x in sat_del_lst}

            self.logger.debug(
                f'SAT ADD PATHS AFTER REMOVING DIFFS: {sat_add_lst}')
            self.logger.debug(
                f'SAT DEL PATHS AFTER REMOVING DIFFS: {sat_del_lst}')
            self.logger.debug(
                f'SAT STILL PATHS AFTER REMOVING DIFFS: {sat_still_lst}')
            self.logger.debug('-----------------------------------')

        sat_add_lst = {str(x)
                       for x in sat_add_lst if self.count_rule_occurrence(x) > 0}
        sat_del_lst = {str(x)
                       for x in sat_del_lst if self.count_rule_occurrence(x) > 0}
        sat_still_lst = {
            str(x) for x in sat_still_lst if self.count_rule_occurrence(x) > 0}
        ...
        self.logger.debug(
            f'LENGTHS OF SAT_ADD, SAT_DEL, SAT_STILL: {sat_add} {sat_del} {sat_still}')

        if self.save_csvs:
            self._save_bdd_paths(class_id, sat_add_lst, 'add')
            self._save_bdd_paths(class_id, sat_del_lst, 'del')
            self._save_bdd_paths(class_id, sat_still_lst, 'still')
        ...
        # Calculate KPIs
        self.results[class_id]['add'] = self.calculate_kpi(
            sat_add, sat_del, sat_still)
        self.results[class_id]['del'] = self.calculate_kpi(
            sat_del, sat_add, sat_still)
        self.results[class_id]['still'] = self.calculate_kpi(
            sat_still, sat_add, sat_del)
        self.results[class_id]['j'] = jaccard_distance(
            set_features_f, set_features_g)

        self.results[class_id]['s1'] = str(len(set_features_f))
        self.results[class_id]['s2'] = str(len(set_features_g))
        self.results[class_id]['union'] = str(
            union(set_features_f, set_features_g))
    ...
    def dict2bdd_paths(dct: Dict) -> List:
        ...
        for path in dct:
            ...
            try:
                temp_path = {i.split(': ')[0]: i.split(': ')[1]
                             for i in temp_path.split(', ')}
            except IndexError:
                continue
            criteria_list = []
    ...
    def count_rule_occurrence(self, rule):
        '''Count the occurrence # of a rule in both datasets.
        '''
        rule = str(rule)
        count_t1 = self.data_manager['left'].count_rule_occurrence(rule)
        count_t2 = self.data_manager['right'].count_rule_occurrence(rule)
    ...
    def calculate_kpi_global(df):
        ...
        df['still_global'] = [
            x if x == x else 0 for x in df['sat_still'] / max_sat_still]
        return df
    ...
    def start_comparison(self, class_id):
        ...
        with stopit.ThreadingTimeout(72000) as to_ctx_mgr:
            ...
            self.logger.info(
                f'Finishing class {class_id}, time: {self.results[class_id]["runtime"]}')
    ...
    def BDD2Text(self, classes=None):
        ...
        def create_BDD2Text(class_id):
            text = BDD2Text(
                f'{self.save_path}/paths_add_del.csv', class_id, 85)
            text.simple_text()
        if not classes:
    ...
    def _save_results(self):
        ...
        kpi_df = pd.DataFrame.from_dict(
            self.results, orient='index').reset_index()
        kpi_df = kpi_df.rename(columns={'index': 'class_id'})
        kpi_df = self.calculate_kpi_global(kpi_df)
        cols = ['class_id', 'add', 'del', 'still', 'add_global', 'del_global',
                'still_global', 'sat_add', 'sat_del', 'sat_still', 'j', 's1',
                's2', 'union', 'runtime']
        kpi_df = kpi_df[cols]
    ...
    def _save_bdd_paths(self, class_id, bdd, type_):
        '''Save the csv with paths for each kpi
        '''
        for rule in bdd:
            count_tot = str(self.count_rule_occurrence(rule))
