    ...
    def simple_text(self):
        ...
        def list_to_string(data, add_feature=True):
            ...
            if len(data) > 1:
                ...
                last = data[-1]
                for i in data_1:
                    i = text_formatter(i, bold=True)
                    t += f'{i}, '
                last = text_formatter(last, bold=True)
                t += f'and {last}'
                if add_feature:
                    t += ' features'
            elif len(data) == 1:
                t = text_formatter(data[0], bold=True)
                if add_feature:
                    t += ' feature'
            else:
                t = 'XXXX'
        ...
        def rule_format(lst, text_type):
            leq_variables = defaultdict(list)
            other_variables = []
            for x in lst:
                if 'LEQ' in x:
                    leq_variables[x.split('LEQ')[0]].append(int(x.split('LEQ')[1]))
                else:
                    other_variables.append(x)
            leq_variables = {k: min(v) if text_type=='pos' else max(v) for k, v in leq_variables.items()}
            lst = [f'{k} <= {v}* Quantile' for k, v in leq_variables.items()]
            lst += other_variables
            return lst
        ...
        def list_to_string_2(data, text_type):
            ...
            """
            t = data
            data = rule_format(data, text_type)
            if text_type == 'pos':
                tc = 10 # green
            else:
                tc = 9 # red
            if len(data) > 1:
                ...
                last = data[-1]
                for i in data_1:
                    i = text_formatter(i, tc=tc, bold=True)
                    t += f'{i}, '
                last = text_formatter(last, tc=tc, bold=True)
                t += f'and {last}'

            elif len(data) == 1:
                t = text_formatter(data[0], tc=tc, bold=True)
        ...
        def rules_to_shared(data):
            ...
            if len(data) > 1:
                if sum(nums) == 0:
                    return f'the document must {text_formatter("not", tc=9, underline=True)} contain {list_to_string_2(words, "neg")}.'
                if sum(nums) == 1:
        ...
        for kind in ['add', 'del', 'still']:
            ...
            if tot_num == 0:
                ...
                colored_title_text = text_formatter(title_text, bc=color)
                print(f"There are no '{colored_title_text}' classification rules.\n ")
                continue
            ...
            to_print = f'This class has {tot_num} {title_text.lower()} classification {rule_rules}.'
            ...
            if sum(Ns) <= 4:
                ...
                for ni in num_list:
                    item = used_paths[ni]
                    rem = text_formatter('Having', tc=10)
                    rem2 = text_formatter('not', tc=9, underline=True)
