...
class FairTreeSurrogate(merlin.surrogate.generic_surrogate.GenericSurrogate):
    '''
    '''
    ...
    def fit(self):
        ...
        np.random.seed(42)
        random.seed(42)
        self._model = self._build_tree(pd.concat(
            [self.X, pd.DataFrame(self.predicted_labels)], axis=1).values.tolist())
        self.score()
    ...
    def score(self):
        ...
        '''
        self.surrogate_predictions = [self.predict(
            x[1], self._model) for x in self.X.iterrows()]
